...
var editContent_ApiCall = function(campaignId, callback) {
  try {
    ...
    var data = {"html": "<p>The HTML to use for the saved campaign<./p>"};
...
Meteor.methods({
  ...
  editContent: function (campaignId) {
    if (Meteor.isServer) {
      this.unblock();
      var response = Meteor.wrapAsync(editContent_ApiCall)(campaignId);
      return response;
    }
  },
